...
const FavCuisine = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      await historyServ.saveFavCuisine({
        userId: user._id,
        option: filtered,
      });
      setTimeout(() => history.push("/nonfavcuisine"), 1000);
    } catch (error) {
      console.log(error);
    } finally {
      fetchOption();
      message.success("Your Favourite Options has been saved");
    }
  };

  const fetchOption = async () => {
    try {
      const res = await historyServ.getFavCuisine(user._id);

      res.map((item) =>
        setOption((prevail) => {
          return [
            ...prevail,
            { itemId: item.itemId, favCuisine: item.favCuisine },
          ];
        })
      );

      let merged = [];
      for (let i = 0; i < favCuisineData.length; i++) {
        merged.push({
          ...favCuisineData[i],
          ...res.find((itemInner) => itemInner.itemId === favCuisineData[i].id),
        });
      }
  ...
  return (
    <div className="dietaryRestriction">
      <div className="title">
        <h1>{user.userName} Profile - FAVOURITE CUISINES </h1>
      </div>

      <div className="dietHeader">
        <h1>FAVOURITE CUISINES</h1>
        <p>Select from the options below your list of FAVOURITE CUISINES.</p>
      </div>
